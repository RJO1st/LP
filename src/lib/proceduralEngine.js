...
// Shuffle a static MCQ template so correct answer isn't always opts[0]
...
// ─── DYNAMIC TEMPLATE PARSER (unchanged) ────────────────────────────────────
...
// ─── INTERACTIVE EXPLANATION TEMPLATES (unchanged) ───────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// MATHS GENERATOR
// Year 1-2: unchanged from original (working well)
// Year 3-6: completely rewritten with rich, year-specific content
// ─────────────────────────────────────────────────────────────────────────────
export const generateLocalMaths = (year, difficultyMultiplier = 1) => {

  // ── YEAR 1 & 2 (original logic, working well) ────────────────────────────
  if (year <= 2) {
    ...
    if (questionType < 0.25) {
      ...
      // FIX: pass proper visual object instead of emoji string
      ...
      // FIX: pass proper visual object (a=whole, b=taken, ans=remaining)
  ...
  if (year === 4) {
    ...
    if (r < 0.25) {
      ...
      const d1o = fmt(dec + 0.1), d2o = fmt(dec + 0.25), d3o = fmt(Math.max(0.05, dec - 0.1));
      const opts = shuffle([String(dec), String(d1o), String(d2o), String(d3o)]);
    ...
    const w1 = typeof ans==='number' ? ans+rand(2,5) : ans, w2 = typeof ans==='number' ? Math.max(1,ans-rand(1,3)) : ans, w3 = typeof ans==='number' ? ans+10 : ans;
    const opts = shuffle([String(ans), String(w1), String(w2), String(w3)]);
  ...
  if (year === 5) {
    ...
    if (r < 0.2) {
      ...
      return { q, opts, a: opts.indexOf(String(p2)), exp:`A prime has exactly 2 factors: 1 and itself. ${p2} is prime. ${c1} is not (${c1} ÷ ${c1%2===0?2:3} = ${c1%(c1%2===0?2:3)===0?c1/(c1%2===0?2:3):''}).`, subject:'maths', topic, hints:["A prime has exactly 2 factors."], vars:{a:p2,b:c1} };
    } else if (r < 0.85) {
      // Area of triangles
      ...
      const lcm = d2; // d2 is always multiple of d1 in our choices
      const equiv1 = n1*(lcm/d1), num_ans = equiv1+n2;
      const whole = Math.floor(num_ans/lcm), rem2 = num_ans%lcm;
...
// ─────────────────────────────────────────────────────────────────────────────
// REAL WORLD MATHS (unchanged — works well)
// ─────────────────────────────────────────────────────────────────────────────
export const generateRealWorldMaths = (year, difficultyMultiplier = 1) => {
  ...
  let w1=weeksNeeded+1, w2=Math.max(1,weeksNeeded-1), w3=weeksNeeded+2;
...
const PASSAGES_Y56 = [
  ...
  },
  {
    ...
    questions: [
      ...
      { q: "Why does the author write 'she had to believe' in brackets in the final sentence?", opts: ["To suggest her belief is fragile and she needs it to survive", "To show she is not sure what she believes", "To indicate she is speaking out loud", "To create a list of her beliefs"], a: 0, exp: "The parenthetical 'she had to believe' hints at doubt — her faith is something she clings to out of necessity, making her more sympathetic and complex." },
    ]
  },
  {
    ...
    questions: [
      ...
      { q: "What type of figurative language is used in 'his great iron head, shaped like a dustbin but as big as a bedroom'?", opts: ["Simile", "Metaphor", "Personification", "Alliteration"], a: 0, exp: "A simile compares using 'like' or 'as'. Both comparisons here use 'as' — making this a double simile that creates a vivid, almost comical scale." },
...
// Get a passage and a question from it, ensuring variety
let _passageIndex = -1;
const getPassageQuestion = (year) => {
  const pool = year >= 5 ? PASSAGES_Y56 : PASSAGES_Y56.slice(0, 3); // Y3-4 get simpler passages
...
// ─────────────────────────────────────────────────────────────────────────────
// ENGLISH GENERATOR — Year-specific, rich content
// ─────────────────────────────────────────────────────────────────────────────
export const generateLocalEnglish = (year) => {
  ...
  if (year === 5) {
    // 40% chance of a reading comprehension passage question
    if (r < 0.4) return getPassageQuestion(5);
    const r2 = Math.random(); // re-roll for grammar/figurative/register
    if (r2 < 0.3) {
      const qs = [
  ...
  // Year 6
  // 40% chance of reading comprehension passage
...
// ─────────────────────────────────────────────────────────────────────────────
// VERBAL REASONING — Year-specific content
// ─────────────────────────────────────────────────────────────────────────────
export const generateLocalVerbal = (year) => {
  ...
  if (year === 4) {
    if (r < 0.35) {
      // Number sequences with varying rules
      const types = [
  ...
  if (year === 5) {
    ...
    if (r < 0.6) {
      ...
      return shuffleTemplate({ ...pick(synonymPairs), subject:'verbal', topic:'synonyms_antonyms' });
    }
  ...
  if (r < 0.3) {
    const qs = [
      ...
      { q:"Code: shift back 3. What does GRB decode to?", opts:["DOY","HUE","FQA","EPS"], a:0, exp:"G-3=D, R-3=O, B-3=Y. The decoded message is DOY. Wait — G(7)-3=D(4), R(18)-3=O(15), B(2)-3=Y(25 wraparound). So DOY." },
      { q:"Mirror code: A↔Z, B↔Y, C↔X... Encode MATHS.", opts:["NZGSH","NZGSW","NZTHS","NZTSH"], a:1, exp:"M(13)→N(14)? No — mirror: M is 13th, mirror = Z-13+1 = 14th from end = N. A→Z, T→G, H→S, S→H. MATHS → NZGSH." },
...
// ─────────────────────────────────────────────────────────────────────────────
// NVR GENERATOR — Year-specific content
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// AI QUESTION GENERATION (unchanged)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// SESSION GENERATOR — 3-Tier Architecture
//
//  Tier 1 — Supabase DB (AI-generated, LRU-ordered, scholar-deduped)
//  Tier 2 — Procedural templates (instant, varied, year-appropriate)
//  Tier 3 — On-the-fly AI generation (last resort, async, fills gaps)
//
// ─────────────────────────────────────────────────────────────────────────────

// Helper: map a DB row to a question object
...
// Tier 3: on-the-fly AI generation (small batch, used only when DB is sparse)
...
export const generateSessionQuestions = async (year, region, count, proficiency, subject, mistakes = [], previousQuestionIds = []) => {
  ...
  for (const { s, n } of mix) {
    ...
    // ── TIER 1: Supabase — LRU-ordered, scholar-deduped ──────────────────────
    try {
      if (supabase) {
        // Request 4× what we need so we have plenty after dedup filtering
        ...
        if (!error && data && data.length > 0) {
          // Filter out questions this scholar has already seen
          const fresh = data.filter(row => !usedIds.has(row.id));

          // Shuffle the fresh pool so LRU order doesn't always produce the same sequence
          const candidates = shuffle(fresh).slice(0, n);
          ...
          // Fire-and-forget: update last_used so these questions rotate to the back
          const ids = subjectQuestions.map(q => q.id).filter(Boolean);
          if (ids.length > 0) {
            supabase.from('question_bank')
              .update({ last_used: new Date().toISOString(), times_used: supabase.rpc ? undefined : undefined })
              .in('id', ids)
              .then(() => {});
          }
        }
    ...
    // ── TIER 2: Procedural — instant, deterministic variety ───────────────────
    const afterTier1 = n - subjectQuestions.length;
    if (afterTier1 > 0) {
      for (let i = 0; i < afterTier1; i++) {
        if (s === 'maths') {
          subjectQuestions.push(Math.random() > 0.7 ? generateRealWorldMaths(year) : generateLocalMaths(year));
    ...
    // ── TIER 3: AI fallback — only if DB is empty AND procedural is exhausted ─
    // This is a safety net — in practice Tier 1+2 should always supply enough.
    // Runs async so it doesn't block the UI.
    ...
    if (afterTier2 > 0) {
      try {
        ...
        // Silently ignore — Tier 2 already handled it
      }
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// FETCH CLAUDE RESPONSE (unchanged)
// ─────────────────────────────────────────────────────────────────────────────
