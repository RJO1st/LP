...
const CERT_MILESTONES = [
  ...
  { threshold: 12, label: "Platinum", emoji: "💎", colour: "#7c3aed" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{
        ...
        border: `1px solid ${overallLevel.current?.colour ?? "#7c3aed"}18`,
      }}>
        <div>
          <div style={{ fontSize: 9, fontWeight: 800, color: "#7c3aed", textTransform: "uppercase", letterSpacing: "0.12em" }}>
            Certificate Level
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
            <span style={{ fontSize: 20 }}>{overallLevel.current?.emoji ?? "🎓"}</span>
            <span style={{ fontSize: 16, fontWeight: 900, color: overallLevel.current?.colour ?? "#7c3aed" }}>
              {overallLevel.current?.label ?? "Beginner"}
            </span>
          </div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 24, fontWeight: 900, color: "#1e1b4b" }}>{totalMastered}</div>
          <div style={{ fontSize: 9, fontWeight: 600, color: "rgba(30,27,75,0.4)" }}>topics mastered</div>
        </div>
      </div>
      ...
      {overallLevel.next && (
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#1e1b4b" }}>
          ...
          <div key={sp.subject} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#1e1b4b" }}>
                  {sp.displayName.split(" ")[0]}
                </span>
                <span style={{ fontSize: 9, fontWeight: 700, color: sp.currentCert?.colour ?? "#7c3aed" }}>
                  {sp.mastered}/{sp.total}
                </span>
              </div>
              <div style={{ height: 3, borderRadius: 2, background: "rgba(124,58,237,0.08)", marginTop: 4, overflow: "hidden" }}>
                <div style={{
                  height: "100%", borderRadius: 2,
                  width: `${sp.pct}%`,
                  background: sp.currentCert?.colour ?? "#7c3aed",
                  transition: "width 0.6s ease",
                }} />
              </div>
              {sp.nextCert && sp.toNextCert > 0 && (
                <div style={{ fontSize: 8, color: "rgba(30,27,75,0.35)", marginTop: 2, fontWeight: 600 }}>
