    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 800, color: "#8b5cf6", textTransform: "uppercase", letterSpacing: "0.15em" }}>
              Mock Test Debrief
            </div>
            <h2 style={{ fontSize: 20, fontWeight: 900, color: isDark ? "#ede9fe" : "#1e1b4b", margin: "4px 0 0" }}>
              {mockResult.subject?.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase()) ?? "Assessment"} Results
            </h2>
          </div>
          {onClose && (
            <button onClick={onClose} style={{
              padding: "6px 14px", borderRadius: 8, border: "1px solid rgba(139,92,246,0.15)",
              background: "rgba(139,92,246,0.04)", color: "#8b5cf6", fontSize: 11, fontWeight: 700, cursor: "pointer",
            }}>
              Close
            </button>
          )}
      ...
      <div style={{ padding: "20px 24px", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        <div style={{ textAlign: "center", padding: 12, borderRadius: 10, background: `${pct >= 70 ? "#22c55e" : pct >= 50 ? "#f59e0b" : "#ef4444"}08`, border: `1px solid ${pct >= 70 ? "#22c55e" : pct >= 50 ? "#f59e0b" : "#ef4444"}15` }}>
          <div style={{ fontSize: 28, fontWeight: 900, color: isDark ? "#ede9fe" : "#1e1b4b" }}>{score}/{total}</div>
          <div style={{ fontSize: 9, fontWeight: 700, color: pct >= 70 ? "#22c55e" : pct >= 50 ? "#f59e0b" : "#ef4444", textTransform: "uppercase" }}>Score</div>
        </div>
        <div style={{ textAlign: "center", padding: 12, borderRadius: 10, background: "rgba(139,92,246,0.04)", border: "1px solid rgba(139,92,246,0.1)" }}>
          <div style={{ fontSize: 28, fontWeight: 900, color: "#8b5cf6" }}>{pct}%</div>
          <div style={{ fontSize: 9, fontWeight: 700, color: "#8b5cf6", textTransform: "uppercase" }}>Accuracy</div>
        </div>
        <div style={{ textAlign: "center", padding: 12, borderRadius: 10, background: "rgba(139,92,246,0.04)", border: "1px solid rgba(139,92,246,0.1)" }}>
          <div style={{ fontSize: 28, fontWeight: 900, color: isDark ? "#ede9fe" : "#1e1b4b" }}>{predictedGrade}</div>
          <div style={{ fontSize: 9, fontWeight: 700, color: "#8b5cf6", textTransform: "uppercase" }}>Predicted</div>
        </div>
        <div style={{ textAlign: "center", padding: 12, borderRadius: 10, background: "rgba(139,92,246,0.04)", border: "1px solid rgba(139,92,246,0.1)" }}>
          <div style={{ fontSize: 28, fontWeight: 900, color: isDark ? "#ede9fe" : "#1e1b4b" }}>{analysis.topics.length}</div>
          <div style={{ fontSize: 9, fontWeight: 700, color: "#8b5cf6", textTransform: "uppercase" }}>Topics</div>
        </div>
      </div>
      ...
      <div style={{ padding: "0 24px 20px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div style={{ padding: 14, borderRadius: 10, background: "rgba(34,197,94,0.04)", border: "1px solid rgba(34,197,94,0.1)" }}>
          ...
          {analysis.strengths.length === 0 ? (
            <div style={{ fontSize: 11, color: isDark ? "rgba(167,139,250,0.4)" : "rgba(30,27,75,0.4)", fontStyle: "italic" }}>Keep practising to build strengths</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {analysis.strengths.slice(0, 4).map(t => (
                <div key={t.topic} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: isDark ? "#ede9fe" : "#1e1b4b" }}>{t.displayName}</span>
        ...
        <div style={{ padding: 14, borderRadius: 10, background: "rgba(239,68,68,0.04)", border: "1px solid rgba(239,68,68,0.1)" }}>
          ...
          {analysis.weaknesses.length === 0 ? (
            <div style={{ fontSize: 11, color: isDark ? "rgba(167,139,250,0.4)" : "rgba(30,27,75,0.4)", fontStyle: "italic" }}>No weak areas — excellent!</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div style={{ padding: "0 24px 20px" }}>
        <button
          ...
          style={{
            width: "100%", padding: "8px", borderRadius: 8, border: "1px solid rgba(139,92,246,0.1)",
            background: "rgba(139,92,246,0.02)", color: "#8b5cf6", fontSize: 11, fontWeight: 700, cursor: "pointer",
            textAlign: "center",
          }}
        >
          ...
          <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 4 }}>
            {analysis.topics.map(t => (
              <div key={t.topic} style={{
                ...
                <span style={{ flex: 1, fontSize: 11, fontWeight: 600, color: isDark ? "#ede9fe" : "#1e1b4b" }}>{t.displayName}</span>
                <span style={{ fontSize: 10, fontWeight: 700, color: t.colour }}>{t.correct}/{t.total}</span>
                {t.mastery != null && (
                  <span style={{ fontSize: 9, color: isDark ? "rgba(167,139,250,0.35)" : "rgba(30,27,75,0.35)" }}>Mastery: {t.mastery}%</span>
                )}
                {t.status === "weakness" && (
                  <button
                    onClick={() => onReviseTopic?.(t.topic, mockResult.subject)}
      ...
      <div style={{
        ...
        {onRetake && (
          <button onClick={onRetake} style={{
            padding: "8px 20px", borderRadius: 10, border: "1px solid rgba(139,92,246,0.2)",
            background: "rgba(139,92,246,0.04)", color: "#8b5cf6", fontSize: 12, fontWeight: 700, cursor: "pointer",
          }}>
            Retake Mock
          </button>
        )}
